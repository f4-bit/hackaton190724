...
# Activar la conversión automática entre pandas DataFrames y R DataFrames
pandas2ri.activate()
...
write.xlsx(misdatos, file = "misdatos.xlsx")
...
negative_words <- sentiment_word_counts %>%
  filter(sentiment == "negative") %>%
  arrange(desc(count)) %>%
  head(20)

# Imprimir palabras negativas del texto principal
...
coments_data %>% 
  filter(lemma != "im", lemma != "w", lemma != "pm", lemma != "n") %>%  # Filtra la palabra "n"
  count(lemma, sort = TRUE) %>%
  mutate(lemma = reorder(lemma, n)) %>%
  select(word = lemma, freq = n) %>% 
  wordcloud2()
...
# Guardar el data frame como un archivo Excel
write.xlsx(sentiment_word_counts, file = "misdatos1.xlsx")
"""
